...
def do_dummy(s: socket.socket) -> None:
    ...
    header = struct.pack("=IIQ", OP_DUMMY, 8, n)
    ...
    assert res[0] == OP_DUMMY, (
        f"request operation does not match. expected {OP_DUMMY}; got {res[0]}"
    )
    assert res[1] == 8, f"response size must be 8; got {res[1]}"
    assert res[2] == n, f"request id expected {id}; got {res[2]}"
    assert res[3] == 0x1, f"response status expected 0x1; got {res[3]}"
    assert res[4] == n, f"expected {n}; got {res[4]}"
