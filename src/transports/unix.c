...
xrpc_transport_server_unix_init(struct xrpc_transport **s,
  ...
  if (ret < 0 && errno != ENOENT)
    XRPC_PRINT_SYSCALL_ERR_AND_RETURN("unlink", XRPC_TRANSPORT_ERR_UNLINK);
...
static int xrpc_transport_server_unix_accept_connection(
  ...
  if (!c) XRPC_PRINT_SYSCALL_ERR_AND_RETURN("malloc", XRPC_API_ERR_ALLOC);
...
static int
xrpc_transport_server_unix_recv(struct xrpc_transport_connection *conn, void *b,
                                size_t l) {
...
static int
xrpc_transport_server_unix_send(struct xrpc_transport_connection *conn,
...
const struct xrpc_transport_ops xrpc_transport_unix_ops = {
    ...
    .send = xrpc_transport_server_unix_send,
    .recv = xrpc_transport_server_unix_recv,
